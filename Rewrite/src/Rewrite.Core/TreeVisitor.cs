...
public abstract class TreeVisitor<T, P> : ITreeVisitor<T, P> where T : class, Tree
{
    ...
    private static Type VisitorTreeType(Type type)
    {
        ...
        {
            type = type.BaseType!;
        } while (type is { BaseType: not null, IsGenericType: true } && type.GetGenericTypeDefinition() != typeof(TreeVisitor<,>) && type.GetGenericTypeDefinition() != typeof(TreeVisitorAsync<,>));
