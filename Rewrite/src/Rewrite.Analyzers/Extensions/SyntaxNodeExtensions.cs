...
using Rewrite.Analyzers.Analyzers;
...
internal static class SyntaxNodeExtensions
{
    ...
    internal static readonly SyntaxTriviaList NullableTrivia = SyntaxFactory.TriviaList(
    ...
    private static readonly SyntaxTrivia AutoGeneratedComment = SyntaxFactory.Comment("// <auto-generated/>");
    ...
    public static SyntaxKind GetAccessibilityModifier<T>(this IEnumerable<T> typeDeclaration) where T : BaseTypeDeclarationSyntax
    {
        
        var modifiers = new SyntaxTokenList(typeDeclaration.SelectMany(x => x.Modifiers));
    ...
    {
        var declaration = SyntaxFactory.ClassDeclaration(type.Identifier.Text)
            .WithModifiers(
                SyntaxFactory.TokenList(
                    SyntaxFactory.Token(type.GetAccessibilityModifier()),
    ...
    {
        var declaration = SyntaxFactory.StructDeclaration(type.Identifier.Text)
            .WithModifiers(
                SyntaxFactory.TokenList(
                    SyntaxFactory.Token(type.GetAccessibilityModifier()),
                    SyntaxFactory.Token(SyntaxKind.PartialKeyword)
    ...
    {
        var declaration = SyntaxFactory.InterfaceDeclaration(type.Identifier.Text)
            .WithModifiers(
                SyntaxFactory.TokenList(
                    SyntaxFactory.Token(type.GetAccessibilityModifier()),
                    SyntaxFactory.Token(SyntaxKind.PartialKeyword)
    ...
    {
        var newNamespace = SyntaxFactory.FileScopedNamespaceDeclaration(@namespace)
            .WithMembers(
                SyntaxFactory.SingletonList(innerMember)
            );
        if (usings.Any())
        {
