...
//         builder.AppendLine($"public partial class {classSymbol.Name}{GetTypeParameters(classSymbol)} : {string.Join(", ", interfaces.Select(x => $"{x}<{classSymbol.ToDisplayString(SymbolDisplayFormat.FullyQualifiedFormat)}>"))}");
//         builder.AppendLine("{}");
...
//
//         // Generate the generic version of the interface
//         builder.Append($"public partial interface {typeSymbol.Name}<out T> : {typeSymbol.ToDisplayString(SymbolDisplayFormat.FullyQualifiedFormat)}");
//
...
//         {
//             builder.Append($", {parentInterface.ToDisplayString(SymbolDisplayFormat.FullyQualifiedFormat)}<T>");
//         }
//
//         builder.AppendLine($" where T : {typeSymbol.ToDisplayString(SymbolDisplayFormat.FullyQualifiedFormat)}");
...
//             {
//                 var methodSignature = $"{typeSymbol.ToDisplayString(SymbolDisplayFormat.FullyQualifiedFormat)} {method.Name}{GetTypeParameters(method)}({GetParameterList(method)})";
//                 if (method.ReturnType.Equals(baseInterface, SymbolEqualityComparer.Default) && !distinctMethods.Contains(methodSignature))
//                 {
//                     builder.AppendLine($"    public new {methodSignature} => ({typeSymbol.ToDisplayString(SymbolDisplayFormat.FullyQualifiedFormat)})(({baseInterface.ToDisplayString(SymbolDisplayFormat.FullyQualifiedFormat)})this).{method.ToDisplayString(SymbolDisplayFormat.FullyQualifiedFormat)}({GetArgumentList(method)});");
...
//             return "";
//         return $"<{string.Join(", ", method.TypeParameters.Select(p => p.ToDisplayString(SymbolDisplayFormat.FullyQualifiedFormat)))}>";
//     }
...
//     {
//         return string.Join(", ", method.Parameters.Select(p => $"{p.Type.ToDisplayString(SymbolDisplayFormat.FullyQualifiedFormat)!.Replace("?","")} {p.Name}"));
//     }
//
//     private string GetArgumentList(IMethodSymbol method)
//     {
//         return string.Join(", ", method.Parameters.Select(p => p.ToDisplayString(SymbolDisplayFormat.FullyQualifiedFormat)));
//     }
// }
//
